...
const TracksTable = ({
  ...
  const fetchTracks = async () => {
    try {
      ...
      let query = supabase.from('spotify_liked').select('*', {
        count: 'exact'
      });
      ...
      let countQuery = supabase.from('spotify_liked').select('*', {
        count: 'exact',
        head: true
      });
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      let artistQuery = supabase.from('spotify_liked').select('artist').not('artist', 'is', null);
      if (selectedGenre) {
      ...
      let genreQuery = supabase.from('spotify_liked').select('genre').not('genre', 'is', null);
      if (noSuperGenre) {
      ...
      const {
        data: superGenreData
      } = await supabase.from('spotify_liked').select('super_genre').not('super_genre', 'is', null);
      if (superGenreData) {
  ...
  return <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span className="flex items-center gap-2">Liked Songs (4932 tracks)<svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
      ...
      <CardContent>
        ...
        {tracks.length > 0 ? <div className="rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-[300px]">Track</TableHead>
                  <TableHead className="cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('artist')}>
                    <div className="flex items-center gap-1">
                      Artist
                      {sortField === 'artist' && (sortDirection === 'asc' ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
                    </div>
                  </TableHead>
                  ...
                  <TableHead className="cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('year')}>
                    <div className="flex items-center gap-1">
                      Year
                      {sortField === 'year' && (sortDirection === 'asc' ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
                    </div>
                  </TableHead>
              ...
              <TableBody>
                {tracks.map(track => <TableRow key={track.id} className={`cursor-pointer ${selectedTrack?.id === track.id ? 'bg-muted' : ''}`} onClick={() => onTrackSelect(track)}>
                    ...
                    <TableCell>
                      {track.super_genre ? <Badge variant="default" className="bg-blue-500/10 text-blue-400 border-blue-500/30">{track.super_genre}</Badge> : <Badge variant="outline" className="bg-gray-500/10 text-gray-400 border-gray-500/30">Unmapped</Badge>}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        {track.genre ? <Badge variant="secondary" className="bg-purple-500/10 text-purple-400 border-purple-500/30">{track.genre}</Badge> : <Badge variant="outline" className="bg-gray-500/10 text-gray-400 border-gray-500/30">No Genre</Badge>}
                        {track.genre && hasOverride(track.genre) && <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/30 text-xs">
                            Overridden
                          </Badge>}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      {track.spotify_id ? <div className="flex items-center gap-2">
                          <Button variant="ghost" size="sm" asChild>
                            <a href={`https://open.spotify.com/track/${track.spotify_id}`} target="_blank" rel="noopener noreferrer" onClick={e => handleSpotifyClick(e, track.spotify_id)} title="Open in Spotify (Ctrl+Click for new tab)">
                              <ExternalLink className="h-4 w-4" />
                            </a>
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => handleCopySpotifyLink(track.spotify_id)} title="Copy Spotify link">
                          ...
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="sm" disabled onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                              <ExternalLink className="h-4 w-4" />
                            </Button>
                          </TooltipTrigger>
                          <TooltipContent>
