...
const BrandLogo: React.FC<BrandLogoProps> = ({ size = 40, className = '' }) => {
  ...
  return (
    <div 
      ...
      style={{ width: size, height: size }}
    >
      {/* Simple white circle background with flip animation on hover */}
      <div 
        className="relative w-full h-full rounded-full overflow-hidden bg-white border border-gray-200 shadow-lg transition-transform duration-700 ease-in-out group-hover:[transform:rotateY(360deg)] [transform-style:preserve-3d]"
        style={{ width: size, height: size }}
      >
        <img
          src={makoSharkImage}
